...
`open()` と `close()` は `onEvent` / `onError` の完了を待たない。`close()` は Source の停止まで待ち、コールバック内からも await できる。終了時のコールバック・失敗診断の配送まで待つ場合は、コールバック外で `await running.close(); await running.drain()` とする。pull stream は配送済みコールバックの診断まで流して終了する。
